...
export default function NouvelleProposition() {
  const [form, setForm] = useState({
    ...
    // Aide (sélection via catalogue)
    aide_id: AIDES[0].id,
    // Production / toiture — renseigné via la carte
    azimut: '',
    inclinaison: '',
    surface: '',
    production_annuelle: '',
    ...
    // Note interne (jamais envoyée dans le PDF)
    note_interne: '',
  })

  // Photo de fond (upload local, convertie en base64 pour être envoyée au PDF)
  const [photoFond, setPhotoFond] = useState(null) // { dataUrl, name }
  ...
  function buildProposalData() {
    ...
    return {
      ...
      // Photo de fond en base64 — utilisée par le script PDF comme image de couverture
    ...
    <div style={{ maxWidth: 480, margin: '0 auto', padding: '24px 16px 60px', fontFamily: 'Inter, sans-serif' }}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <label style={{ ...labelStyle, marginBottom: 10 }}>
          La carte se centre automatiquement sur l&apos;adresse renseignée ci-dessus. Tracez ensuite le contour de la toiture.
        </label>
        ...
        {/* ── PRODUCTION (entre carte et conso EDF) ── */}
        ...
        {/* ── PHOTO DE FOND (va dans le PDF) ── */}
        <div style={sectionTitleStyle}>Photo de la maison</div>
        <label style={labelStyle}>
          Cette photo sera utilisée en arrière-plan subtil sur la couverture du PDF
        </label>
        <input
          type="file"
          accept="image/*"
        ...
        {/* ── NOTE INTERNE (jamais dans le PDF) ── */}
        <div style={sectionTitleStyle}>Note interne</div>
        <label style={labelStyle}>
          Visible uniquement par vous — n&apos;apparaît jamais dans le PDF envoyé au client
        </label>
        <textarea
          value={form.note_interne}
          onChange={(e) => update('note_interne', e.target.value)}
          placeholder="Contexte du rendez-vous, points d'attention, relances à prévoir…"
          rows={4}
          style={{
            ...inputStyle,
            resize: 'vertical',
